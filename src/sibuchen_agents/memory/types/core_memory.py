...
from sibuchen_agents.memory.base import BaseMemory, MemoryItem, MemoryConfig
from sibuchen_agents.memory.embedding import get_text_embedder, get_dimension
...
class SemanticMemory(BaseMemory):
    ...
    def _init_databases(self):
        ...
        try:
            # 获取数据库配置
            from sibuchen_agents.core.config import qdrant_config, neo4j_config


            # 初始化Qdrant向量数据库（使用连接管理器避免重复连接）
            from sibuchen_agents.memory.storage.qdrant_store import QdrantConnectionManager
            qdrant_config = qdrant_config.get_qdrant_config() or {}
            qdrant_config["vector_size"] = get_dimension()
            self.vector_store = QdrantConnectionManager.get_instance(**qdrant_config)
            logger.info("✅ Qdrant向量数据库初始化完成")

            # 初始化Neo4j图数据库
            from sibuchen_agents.memory.storage.neo4j_store import Neo4jGraphStore
            neo4j_config = neo4j_config.get_neo4j_config() or {}
    ...
    def _combine_and_rank_results(
        ...
        if logger.level <= logging.DEBUG:
            for i, result in enumerate(sorted_results[:3]):
                logger.debug(
                    f"  结果{i + 1}: 向量={result['vector_score']:.3f}, 图={result['graph_score']:.3f}, 精确={result.get('exact_match_bonus', 0):.3f}, 关键词={result.get('keyword_bonus', 0):.3f}, 公司={result.get('company_bonus', 0):.3f}, 实体={result.get('entity_type_bonus', 0):.3f}, 综合={result['combined_score']:.3f}")
    ...
    def _store_linguistic_analysis(self, doc, text: str):
        ...
        try:
            ...
            logger.debug(
                f"🔗 已将词法分析结果存储到Neo4j: {len([t for t in doc if not t.is_punct and not t.is_space])} 个词元")
        ...
        try:
            ...
            for entity_data in related_entities:
                ...
                if not entity_obj:
                    ...
                    related.append({
                        "entity": entity_obj,
                        "relation_type": entity_data.get("relationship_path", ["RELATED"])[-1] if entity_data.get(
                            "relationship_path") else "RELATED",
