...
说明：
- 提供统一的文本嵌入接口与多实现：DashScope（通义千问）、本地Transformer、TF-IDF兜底。
- 暴露 get_text_embedder()/get_dimension()/refresh_embedder() 供各记忆类型统一使用。
- 通过环境变量优先级：DashScope (REST) > DashScope (SDK) > Local (ST) > Local (HF) > TF-IDF。

环境变量：
- EMBED_MODEL_TYPE: "dashscope" | "local" | "tfidf"（默认 dashscope）
- EMBED_MODEL_BASE_URL: Embedding Base URL（统一命名，可选）
- EMBED_MODEL_API_KEY: Embedding API Key（统一命名）
- EMBED_MODEL_ID: 模型名称"default_model"（dashscope默认 text-embedding-v3；local默认 sentence-transformers/all-MiniLM-L6-v2）
"""

from typing import List, Union, Optional
import threading
import numpy as np
from sibuchen_agents.core.config import config
import os
...
def _build_embedder() -> EmbeddingModel:
    preferred = (config.EMBED_MODEL_TYPE or "dashscope").strip()
    # 根据提供商选择默认模型
    model_name = (config.EMBED_MODEL_ID or "").strip()
    kwargs = {}
    # 使用用户指定的模型名称
    if model_name:
        ...
        if preferred == "dashscope":
            kwargs["model_name"] = "text-embedding-v3"
    ...
    base_url = config.EMBED_MODEL_BASE_URL
    if base_url:
