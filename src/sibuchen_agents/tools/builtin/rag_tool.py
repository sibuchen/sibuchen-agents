...
from sibuchen_agents.tools.base import Tool, ToolParameter, tool_action
from sibuchen_agents.memory.rag.pipeline import create_rag_pipeline
from sibuchen_agents.core.llm_client import SibuchenAgentsLLMClient
from sibuchen_agents.core.config import qdrant_config
...
class RAGTool(Tool):
    ...
    def _init_components(self):
        ...
        try:
            ...
            # 初始化 LLM 用于回答生成
            self.llm = SibuchenAgentsLLMClient()
    ...
    def _ask(
        ...
        try:
            ...
            # 兼容 LLMResponse 对象和字符串
            answer_text = answer.content if hasattr(answer, 'content') else str(answer) if answer else ""
    ...
    def get_relevant_context(self, query: str, limit: int = 3, max_chars: int = 1200,
    ...
    def batch_add_texts(self, texts: List[str], document_ids: Optional[List[str]] = None, chunk_size: int = 800,
                        chunk_overlap: int = 100, namespace: Optional[str] = None) -> str:
        """批量添加文本"""
        try:
            if not texts:
                return "❌ 文本列表不能为空"
    ...
    def add_texts_batch(self, texts: List[str], namespace: str = "default",
