...
SUPPORTED_BACKENDS = {
    "hybrid",  # 混合搜索 模式
    "advanced",  # 高级搜索 模式
    "tavily",  # Tavily 搜索引擎（专为 AI 设计）
    "serpapi",  # SerpApi（可抓取 Google 等搜索结果）
    "duckduckgo",  # DuckDuckGo 免费免密钥搜索
    "searxng",  # SearXNG（开源聚合搜索引擎、需要自托管、免费）
    "perplexity",  # Perplexity AI（结合大模型问答）
}
...
class SearchTool(Tool):
    ...
    def __init__(
        ...
        self.tavily_key = tavily_key or config.TAVILY_API_KEY
        self.serpapi_key = serpapi_key or config.SERPAPI_API_KEY
        self.perplexity_key = perplexity_key or config.PERPLEXITY_API_KEY
    ...
    def _search_searxng(
        ...
        host = config.SEARXNG_URL.rstrip("/")
