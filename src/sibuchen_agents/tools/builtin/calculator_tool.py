...
from sibuchen_agents.tools.errors import ToolErrorCode
...
class CalculatorTool(Tool):
    ...
    def _eval_node(self, node):
        ...
        if isinstance(node, ast.Constant):  # Python 3.8+
            ...
            return self.OPERATORS[cast(Any, type(node.op))](
                self._eval_node(node.left),
                self._eval_node(node.right)
            )
        elif isinstance(node, ast.UnaryOp):
            return self.OPERATORS[cast(Any, type(node.op))](self._eval_node(node.operand))
        elif isinstance(node, ast.Call):
            if isinstance(node.func, ast.Name):
                func_name = node.func.id
            else:
                raise ValueError(f"不支持的函数调用类型: {type(node.func)}")
    ...
    def get_parameters(self):
        """获取工具参数定义"""
        from sibuchen_agents.tools.base import ToolParameter
        return [
            ToolParameter(
                name="input",
                type="string",
...
def calculate(expression: str) -> str:
    ...
    """
    tool = CalculatorTool()
    response = tool.run({"input": expression})
