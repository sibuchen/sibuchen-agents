...
class FunctionCallAgent(Agent):
    ...
    def _invoke_with_tools(self, messages: list[dict[str, Any]], tools: list[dict[str, Any]], tool_choice: Union[str, dict], **kwargs):
        ...
        if client is None:
            raise RuntimeError("SibuchenAgentsLLMClient 未正确初始化客户端，无法执行函数调用。")
    ...
    def run(
        ...
        **kwargs,
        ...
        if not tool_schemas:
            ...
            self.add_message(SibuchenMessage("assistant", response_text))
        ...
        self.add_message(SibuchenMessage("user", input_text))
        self.add_message(SibuchenMessage("assistant", final_response))
