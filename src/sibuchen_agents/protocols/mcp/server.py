...
class MCPServer:
    ...
    def add_resource(
        ...
        """
        # 使用装饰器注册资源
        # FastMCP.resource() 要求必须提供 URI，如果未提供则生成一个默认的
        effective_uri = uri or f"mcp://resources/{func.__name__}"
        self.mcp.resource(effective_uri, name=name, description=description)(func)
    ...
    def run(self, transport: Optional[Literal["stdio", "http", "sse", "streamable-http"]] = "stdio", **kwargs):
        """运行服务器
