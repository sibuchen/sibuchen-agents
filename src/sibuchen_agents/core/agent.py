...
from .config import SibuchenConfig, config as _config
from .lifecycle import AgentEvent, EventType, LifecycleHook

if TYPE_CHECKING:
    from sibuchen_agents.tools.registry import ToolRegistry
    from sibuchen_agents.observability.trace_logger import TraceLogger
    from sibuchen_agents.tools.tool_filter import ToolFilter
...
class Agent(ABC):
    ...
    def __init__(
        ...
        # 上下文工程组件
        from sibuchen_agents.context.history import HistoryManager
        from sibuchen_agents.context.truncator import ObservationTruncator
        ...
        # Token 计数器（缓存 + 增量计算）
        from sibuchen_agents.context.token_counter import TokenCounter
        ...
        from sibuchen_agents.skills.loader import SkillLoader
        ...
        if self.config.SKILLS_ENABLED:
            ...
            if self.config.SKILLS_AUTO_REGISTER and self.tool_registry:
                from sibuchen_agents.tools.builtin.skill_tool import SkillTool
    ...
    @property
    def _history(self) -> List[SibuchenMessage]:
        """向后兼容：通过 property 代理到 HistoryManager"""
        return self.history_manager.get_history()

    @_history.setter
    def _history(self, value: List[SibuchenMessage]):
        """向后兼容：允许直接设置历史"""
    ...
    def add_message(self, message: SibuchenMessage):
        """添加消息到历史记录
    ...
    def get_history(self) -> List[SibuchenMessage]:
        """获取历史记录"""
        return self.history_manager.get_history()
    ...
    def _generate_simple_summary(self, history: List[SibuchenMessage]) -> str:
        """生成简单摘要（统计信息）
...
（历史已压缩，保留最近 {self.config.MIN_RETAIN_ROUNDS} 轮完整对话）"""
    ...
    def _generate_smart_summary(self, history: List[SibuchenMessage]) -> str:
        """生成智能摘要（调用 LLM）
...
请用简洁的中文输出，每部分不超过 3 行。"""
        ...
        try:
            ...
            summary = summary_llm.invoke(
                messages,
                temperature=self.config.SUMMARY_LLM_TEMPERATURE,
...
（已压缩，保留最近 {self.config.MIN_RETAIN_ROUNDS} 轮完整对话）"""
    ...
    def _format_history_for_summary(self, history: List[SibuchenMessage]) -> str:
        """格式化历史消息用于摘要生成
    ...
    def _get_summary_llm(self):
        ...
        Returns:
            SibuchenAgentsLLMClient 实例
        """
        if not hasattr(self, '_summary_llm'):
            from sibuchen_agents.core.llm_client import SibuchenAgentsLLMClient

            # 使用配置中的轻量模型
            provider = self.config.SUMMARY_LLM_BASE_URL
            model = self.config.SUMMARY_LLM_MODEL_ID

            self._summary_llm = SibuchenAgentsLLMClient(
                provider=provider,
                model=model,
                temperature=self.config.SUMMARY_LLM_TEMPERATURE,
                max_tokens=self.config.SUMMARY_LLM_MAX_TOKENS
    ...
    def _execute_tool_call(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        if tool:
            try:
                ...
                # 根据状态添加前缀
                from sibuchen_agents.tools.response import ToolStatus
    ...
    def save_session(self, session_name: str) -> str:
        ...
        """
        if not self.session_store:
            raise RuntimeError("会话持久化未启用，请在 SibuchenConfig 中设置 session_enabled=True")
    ...
    def load_session(self, filepath: str, check_consistency: bool = True) -> None:
        ...
        for msg_data in session_data.get("history", []):
            self.history_manager.append(SibuchenMessage.from_dict(msg_data))
    ...
    def _extract_tools_from_history(self, history: List[SibuchenMessage]) -> List[str]:
        """从历史中提取使用的工具
    ...
    def _register_task_tool(self):
        ...
        自动注册逻辑，支持用户自定义工厂函数。
        """
        from sibuchen_agents.tools.builtin.task_tool import TaskTool
        from sibuchen_agents.agents.factory import default_subagent_factory
    ...
    def _register_todowrite_tool(self):
        ...
        """
        from sibuchen_agents.tools.builtin.todowrite_tool import TodoWriteTool
    ...
    def _register_devlog_tool(self):
        ...
        自动注册逻辑，在 __init__ 中调用（如果启用）
        """
        from sibuchen_agents.tools.builtin.devlog_tool import DevLogTool
    ...
    def _create_light_llm(self) -> SibuchenAgentsLLMClient:
        """创建轻量模型 LLM 实例
        ...
        """
        # 复用主 LLM 的配置，但使用轻量模型
        light_llm = SibuchenAgentsLLMClient(
