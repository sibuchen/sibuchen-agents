# SibuchenAgents统一LLM接口 - 支持OpenAI、Anthropic、Gemini等多种接口
...
class SibuchenAgentsLLMClient:
    """
    SibuchenAgents统一LLM客户端

    设计理念：
    - 统一配置：只需 LLM_BASE_URL、LLM_API_KEY、LLM_MODEL_ID、LLM_TIMEOUT
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            temperature: 温度参数，默认从 LLM_TEMPERATURE 读取，默认0.7
            timeout: 超时时间（秒），默认从 LLM_REQUEST_TIMEOUT 读取，默认600秒
            max_tokens: 最大token数，默认从 LLM_MAX_TOKENS 读取
        """
        # 加载配置
        self.base_url = base_url or config.LLM_BASE_URL
        self.api_key = api_key or config.LLM_API_KEY
        self.model = model or config.LLM_MODEL_ID
    ...
    def invoke_with_tools(
        ...
        **kwargs
        ...
        Raises:
            SibuchenAgentsException: 当 LLM 调用失败时
        """
        # 合并参数
        call_kwargs = {
            "temperature": kwargs.pop("temperature", self.temperature),
